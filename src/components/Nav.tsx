  ...
  flex flex-col p-6 text-white'
      >
        <div className='flex justify-between items-center mb-2'>
          ...
          <button className='text-white/80 hover-red-400 text-2xl font-bold transition'
...
rounded-lg transition-all' onClick={async()=>await signOut({callbackUrl:"/"})}>
  ...
  return (
    ...
    to-green-700 rounded-2xl shadow-black/30 flex justify-between items-center h-20 px-4
    md:px-8 z-50'>
      <Link href={"/"} className='text-white font-extrabold text-2xl sm:text-3xl tracking-wide
    hover:scale-105 transition-transform'>
        snapcart
      </Link>
      {user.role == "user" && <form className='hidden md:flex items-center bg-white rounded-full px-4 py-2 w-1/2 max-w-lg
    shadow-md'>
    ...
    shadow-md hover:scale-105 transition md:hidden' onClick={() => setSearchBarOpen((prev) => !prev)}>
        ...
        {user.role == "admin" && <>
          <div className='hidden md:flex items-center gap-4'>
            <Link href={"/admin/add-grocery"} className='flex items-center gap-2 bg-white text-green-700
          ...
          justify-center shadow-md' onClick={() => setMenuOpen(prev => !prev)}>
        ...
        <div className='relative' ref={profileDropDown}>
          <div className='bg-white rounded-full w-11 h-11 flex items-center justify-center
      overflow-hidden shadow-md hover:scale-105 transitionn-transform ' onClick={() => setOpen(prev => !prev)}>
            {user.image ? <Image src={user.image} alt="user" fill className='object-cover rounded-full' /> : <User />}
          </div>
        ...
        border-gray-200 p-3 z-[999]'
              >
                <div className='flex items-center gap-3 px-8 py-2 border-b border-gray-100'>
                  <div className='w-10 h-10 relative rounded-full bg-green-100 flex items-center overflow-hidden'>
                    {user.image ? <Image src={user.image} alt="user" fill className='object-cover rounded-full' /> : <User />}
                ...
                {user.role == "user" &&
                  <Link href={""} className='flex items-center gap-2 px-3 py-3 hover:bg-green-50
          rounded-lg text-gray-400 font-medium' onClick={() => setOpen(false)}>
                    <Package className='w-5 h-5 text-green-600' />
                    My Order

                  </Link>}

                <button className='flex items-center gap-3 w-full text-left px-3 py-3 hover:bg-red-50 rounded-lg
          text-gray-700 font-medium' onClick={() => {
                    setOpen(false)
